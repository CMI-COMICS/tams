...
class AccountController {
  ...
  async create(req, res) {
    try {
      ...
      const response = await this.user.create(username, password, role);
      res.json({
        success: true,
        data: {
  ...
  async login(req, res) {
    try {
      ...
      const user = await this.user.verify(username, password);
      if (!user) {
        return res.json({
          success: false,
      ...
      res.json({
        success: true,
        data: {
          token: jwt.sign({ "username": user.username, "role":  user.role}, process.env.API_SECRET_KEY, {
            expiresIn: "1d",
          }),
        }
      });
